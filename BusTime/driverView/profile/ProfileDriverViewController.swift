...
class ProfileDriverViewController: BaseDriverViewController {
    ...
    var model: UserDetail? {
        didSet {
            ...
            languageView.rightTitle.text = model?.lang == "kz" ? "Қазақша" : "Русский"
        }
    }
    ...
    @objc func tapEdit() -> Void {
        navigationController?.pushViewController(EditProfileDriverViewController(), animated: true)
    }
    ...
    @objc func tapMake() -> Void {
        ...
        ParseManager.shared.multipartFormData(url: api.edit, parameters: parameter) { (result: CheckRequest?, error) in
            ...
            self.showSuccessMessage {
                ...
                LanguageCenter.standard.setLanguageDriver(language: self.languageView.rightTitle.text == "Русский" ? "ru" : "kk-KZ")
            }
        }
    }
...
extension ProfileDriverViewController: UIPickerViewDataSource, UIPickerViewDelegate{
...
// MARK: - Parser
extension ProfileDriverViewController {
