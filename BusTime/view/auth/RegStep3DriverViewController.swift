//
//  RegStep3DriverViewController.swift
//  BusTime
//
...
class RegStep3DriverViewController: ScrollViewController {
    ...
    private var kaspiNumber = String()
    ...
    lazy var stepTitle: UILabel = {
        let label = UILabel()
        label.text = localized(text: "step3")
    ...
    lazy var kaspiField: DefaultTextField = {
        let field = DefaultTextField()
        field.keyboardType = .default
        ...
        field.actionButton.addTarget(self, action: #selector(tapKaspi), for: .touchUpInside)
        return field
    }()
    ...
    func setupViews() -> Void {
        ...
        scrollView.addSubviews([welcomeTitle,justTitle,imageView,nameField,surnameField,typeTransportField,numTransportField,convenienceField, kaspiField])
        ...
        typeTransportField.snp.makeConstraints { (make) in
            make.top.equalTo(kaspiField.snp.bottom).offset(16)
            make.left.equalToSuperview().offset(16)
            make.right.equalToSuperview().offset(-16)
            make.height.equalTo(48)
        }
        ...
        // MARK: - ImageButtons
        scrollView.addSubviews([imageBtn,imageBtn1,imageBtn2,imageBtn3, imageBtn4])
    ...
    @objc func tapKaspi() -> Void {
        let vc = KaspiViewControlller()
        vc.delegate = self
        navigationController?.pushViewController(vc, animated: true)
    }
    @objc func tapConfirm() -> Void {
        if self.kaspiNumber == "" || self.fullName == "" {
            showErrorMessage(messageType: .none, "Заполните данные по номеру каспий")
        }
        //if nameField.text == "" { showErrorMessage(messageType: .none, "Загрузите фото тех паспорта!") }
        else if downloadImage1.cameraImage.image == #imageLiteral(resourceName: "Group-20") { showErrorMessage(messageType: .none, "Загрузите фото тех паспорта!") }
        else if downloadImage2.cameraImage.image == #imageLiteral(resourceName: "Group-20") { showErrorMessage(messageType: .none, "Загрузите фото удостоверения личности!") }
        else if downloadImage3.cameraImage.image == #imageLiteral(resourceName: "Group-20") { showErrorMessage(messageType: .none, "Загрузите фото транспорта!") }
        ...
        else if downloadImage2.cameraImage1.image == #imageLiteral(resourceName: "Group 10484"){ showErrorMessage(messageType: .none, "Загрузите второе фото удостоверения") }
        else if downloadImage3.cameraImage1.image == #imageLiteral(resourceName: "Group 10484") { showErrorMessage(messageType: .none, "Загрузите три фото транспорта!") }
        else if downloadImage3.cameraImage2.image == #imageLiteral(resourceName: "Group 10484") { showErrorMessage(messageType: .none, "Загрузите три фото транспорта!") }
        else if downloadImage4.cameraImage.image == #imageLiteral(resourceName: "Group-20") { showErrorMessage(messageType: .none, "Загрузите аватар транспорта!") }
        else { confirm() }
    }
}

//  MARK: - Picker delegate
extension RegStep3DriverViewController: UIPickerViewDataSource, UIPickerViewDelegate{
...
extension RegStep3DriverViewController: ImagePickerDelegate {
...
extension RegStep3DriverViewController: ConvenienceViewControllerDelegate {
    func convenienceTitle(convenienceTitle: String) {
        convenienceField.text = convenienceTitle
    }
}

// MARK: - KaspiViewController delegate
extension RegStep3DriverViewController: KaspiViewControllerDelegate {
    func kaspiTitle(kaspiTitle: String, fullName:String, kaspiNumber:String) {
        kaspiField.text = kaspiTitle
        self.fullName = fullName
        self.kaspiNumber = String(kaspiNumber.suffix(kaspiNumber.count-2)).replacingOccurrences(of: " ", with: "")
        print(kaspiNumber)
    }
}

// MARK: - Parser
extension RegStep3DriverViewController {
    ...
    private func confirm() -> Void {
        showHUD()
        let avatar: Data = (imageView.image?.jpegData(compressionQuality: 0.1))!
        ...
        let parameter = ["name": nameField.text!,
                         ...
                         "car_image": carImage,
                         "bank_card":self.kaspiNumber,
                         "state_number": numTransportField.text ?? "",
                         "card_fullname":self.fullName,
                         ...
                         "baggage": convenienceField.text?.contains("Багаж") ?? false ? 1 : 0,
                         "code": code!,
                         "device_type": "ios",
                         "device_token": AppDelegate.deviceToken] as Parameters
