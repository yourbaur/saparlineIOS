...
final class PassengerInfoCell: UITableViewCell {
    
    static let identifier = "PassengerInfoCell"
    ...
    public func configureCell(model: PassengerInfoModel, item: Int) {
        placeTextLabel.text = model.seat?[item] ?? ""
        iinTextLabel.text = model.iin ?? ""
        nameTextLabel.text = model.name ?? ""
        phoneTextLabel.text = model.phone ?? ""
    }
}
